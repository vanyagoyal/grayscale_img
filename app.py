...
def upload_image():
    ...
    with open(os.path.join('static/' , filename), 'wb') as img1 :
        img1.write(file_data)
        # ends here
...
def make_grayscale(input_image):
    ...
    array_to_img = cv2.imdecode(image_array , cv2.IMREAD_UNCHANGED)
    print("Decoded Values of the Image : " , array_to_img)

    converted_gray_img = cv2.cvtColor(array_to_img , cv2.COLOR_RGB2GRAY)
    status , output_img = cv2.imencode('.PNG' , converted_gray_img)
